...
var tag_behaviour = "\
	...
	if( typeof(fn) == 'string'){ \
		...
		if(parentname == undefined){ \
			...
			if(fn.level == 0){ \
				innertag = tag(pseudo,arguments.callee.content,id,attrs);	\
				...
				if(fn.level == 1 && deepest){ \
					...
					if(GLOBAL['livehtml'][parentname]['content'][fn.level]){ \
						...
						console.log('level '+fn.level+' '+parent['content'][fn.level]); \
					}\
				} \
			} \
		} \
...
non_self_closing_tags.forEach(function(key) {
	...
	eval('livehtml.'+key+' = function '+key+'(id,attrs,fn){ var glevel = 0; deepest = 0; eval(tag_behaviour); var res = String(arguments.callee.content); reset(key); return res; };');
...
function reset(key){	
	if(Object.keys(livehtml[key])[0]){
		// console.log(Object.keys(livehtml[key])[0]);
		// console.log(key);
